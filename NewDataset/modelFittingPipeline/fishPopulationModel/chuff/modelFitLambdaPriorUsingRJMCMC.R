...
code <- nimbleCode({
  ...
  for(ind in 1:nInds){
    for(age in 1:maxAge){ #loop through the age
      ...
      logit(survivalProb[ind, age]) <- intercept + bsurvAge[age] + bsurvSex[sexIDObs[ind]] + bsurvYear[year[ind]] +
  ...
  for(yr in 1:nYears){
    lambda[yr] ~ dnorm(1, sd = 0.35) # Prior for population growth rate
  }
  ...
  for(ind in 1:nInds){
    ...
    for(age in 2:(maxAge-1)){
      ...
      w[ind, age] <- (g[ind, age-1]/(lambda[year[ind]] -z[ind, age]))*w[ind, age-1]
    }
    w[ind, maxAge] <- (g[ind, maxAge-1]/(lambda[year[ind]] -z[ind, maxAge]))*w[ind, maxAge-1] 
    ...
    ey[ind, 1] <- (lambda[year[ind]] - sum(ey[ind, 2:maxAge]))*fecundicity[ind, 1]
...
constants = list(
  nYears = length(unique(year[1:nObs])),
  year = as.numeric(as.factor(year[1:nObs])),
  nInds = nObs,
  nPreds = nPreds, 
...
inits <- list(
  ...
  bsurvYear = rnorm(length(unique(year[1:nObs])), 0,1),
  sdbsurvSex = 1,
  sdbsurvAge = 1,
  sdbsurvYear = 1,
  lambda = rep(1, length(unique(year[1:nObs]))),
