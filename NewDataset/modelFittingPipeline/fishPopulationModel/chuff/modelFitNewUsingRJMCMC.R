...
setwd("~/Mounts/Egenutvikling/64132_fu_grunnbevilgning_kwaku_adjei/fishyIPMs/NewDataset")
...
###########################
  ...
  for(ind in 1:nInds){
    for(age in 1:maxAge){ #loop through the age
      ...
      logit(survivalProb[ind, age]) <- intercept + bsurvAge[age] + bsurvSex[sexID[ind]] +
                                        bsurvIndex[1] * year[ind]  + 
                                        #bsurvIndex[2] * forest[ind] + 
                                        #bsurvIndex[3] * pastures[ind] + 
                                        bsurvIndex[3] * popnDensity[ind] + 
                                        bsurvIndex[4] * summerTemp[ind] + 
                                        bsurvIndex[2] * summerSnowDepth[ind]  + 
                                        #bsurvIndex[4] * forestsq[ind] +
                                       # bsurvIndex[8] * pasturessq[ind] + 
                                        #bsurvIndex[9] * popnDensitysq[ind] + 
                                        bsurvIndex[5] * summerTempsq[ind] + 
                                        #bsurvIndex[11] * summerSnowDepthsq[ind] + 
                                        bsurvIndex[6] * WPUE[ind] +
                                        bsurvIndex[7] * lakeID[ind] +
                                       # bsurvIndex[14] * sexID[ind]+
                                        bsurvIndex[8] * summerTempsq[ind] * WPUE[ind] + 
                                        bsurvIndex[9] * pow(WPUE[ind], 2) +
                                        bsurvIndex[10] * pow(lakeID[ind], 2) 
        }
  }
...
covs <- data.frame(year = as.numeric(scale(dataList$ageAtHarvestData$year, scale = FALSE)),
                   ...
                   forestsq = as.numeric((scale(dataList$ageAtHarvestData$forest)))^2,
                   ...
                   popnDensitysq = as.numeric((scale(dataList$ageAtHarvestData$popnDensity)))^2,
                   WPUE = wpue,
