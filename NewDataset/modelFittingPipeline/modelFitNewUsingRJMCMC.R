...
###########################
  ...
  for(ind in 1:nInds){
    for(age in 1:maxAge){ #loop through the age
      ...
      logit(survivalProb[ind, age]) <- intercept + bsurvAge[age] + 
                                        bsurvIndex[1]* year[ind]  + 
                                        bsurvIndex[2]*forest[ind] + 
                                        bsurvIndex[3]*pastures[ind] + 
                                        bsurvIndex[4]*popnDensity[ind] + 
                                        bsurvIndex[5]* summerTemp[ind] + 
                                        bsurvIndex[6]*summerSnowDepth[ind]  + 
                                         bsurvIndex[7]*forestsq[ind] +
                                        bsurvIndex[8]*pasturessq[ind] + 
                                        bsurvIndex[9]*popnDensitysq[ind] + 
                                        bsurvIndex[10]*summerTempsq[ind] + 
                                        bsurvIndex[11]*summerSnowDepthsq[ind] + 
                                        bsurvIndex[12]*WPUE[ind] +
                                        bsurvIndex[13]*lakeID[ind] +
...
constants = list(
  ...
  sexID = (as.numeric(dataList$sprawningData$sex)-1),
  nSex = length(unique(as.numeric(dataList$sprawningData$sex))),
  lakeID = (as.numeric(as.factor(dataList$ageAtHarvestData$InnsjoNr))-1),
